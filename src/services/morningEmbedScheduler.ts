...
const getTimeZone = () =>
  process.env.MORNING_EMBED_TIME_ZONE || DEFAULT_TIME_ZONE;
    ...
    get(ZEN_QUOTES_TODAY_URL, (response) => {
      ...
      response.on("end", () => {
        try {
          ...
          console.log("ZenQuotes did not return a valid quote of the day.");
          resolve(null);
        } catch (error) {
          console.log(`There was an error parsing the ZenQuotes response: ${error}`);
          resolve(null);
        }
      });
    }).on("error", (error) => {
      console.log(`There was an error fetching the ZenQuotes daily quote: ${error}`);
      resolve(null);
    });
  });
...
const sendMorningEmbeds = async (client: Client, timeZone: string) => {
  ...
  for (const morningEmbed of morningEmbeds) {
    ...
    try {
      ...
      if (dailyQuote) {
        embed.addFields(
          { name: "Quote", value: dailyQuote.q },
          { name: "Author", value: dailyQuote.a }
        );
      }
